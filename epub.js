...
var src_contents = __webpack_require__(9);
...
// CONCATENATED MODULE: ./src/managers/views/inline.js
...
class inline_InlineView {
  constructor(section, options) {
    ...
    this.id = "epubjs-view:" + Object(core["uuid"])();
  ...
  container(axis) {
    ...
    element.classList.add("epub-view"); // if(this.settings.axis === "horizontal") {
  ...
  create() {
    ...
    this.resizing = true; // this.frame.style.display = "none";
    ...
    this.added = true;
    this.elementBounds = Object(core["bounds"])(this.element);
    return this.frame;
  }
  ...
  render(request, show) {
    ...
    // }.bind(this))
    .then(function () {// this.settings.layout.format(view.contents);
    ...
    .then(function () {
      ...
      this.settings.layout.format(this.contents); // Expand the iframe to the full size of the content
      // this.expand();
      ...
      this.emit(constants["c" /* EVENTS */].VIEWS.RENDERED, this.section);
    }.bind(this)).catch(function (e) {
      this.emit(constants["c" /* EVENTS */].VIEWS.LOAD_ERROR, e);
    }.bind(this));
  ...
  lock(what, width, height) {
    var elBorders = Object(core["borders"])(this.element);
    var iframeBorders;

    if (this.frame) {
      iframeBorders = Object(core["borders"])(this.frame);
    } else {
      iframeBorders = {
        width: 0,
        height: 0
    ...
    if (what == "width" && Object(core["isNumber"])(width)) {
      this.lockedWidth = width - elBorders.width - iframeBorders.width;
      this.resize(this.lockedWidth, false); //  width keeps ratio correct
    }

    if (what == "height" && Object(core["isNumber"])(height)) {
      this.lockedHeight = height - elBorders.height - iframeBorders.height;
      this.resize(false, this.lockedHeight);
    }

    if (what === "both" && Object(core["isNumber"])(width) && Object(core["isNumber"])(height)) {
      this.lockedWidth = width - elBorders.width - iframeBorders.width;
      this.lockedHeight = height - elBorders.height - iframeBorders.height;
  ...
  expand(force) {
    ...
    this._expanding = true; // Expand Horizontally
  ...
  resize(width, height) {
    ...
    this.emit(constants["c" /* EVENTS */].VIEWS.RESIZED, size);
  }

  load(contents) {
    var loading = new core["defer"]();
    var loaded = loading.promise;
    var doc = Object(core["parse"])(contents, "text/html");
    ...
    */
    ...
    this.window = this.document.defaultView;
    this.contents = new src_contents["a" /* default */](this.document, this.frame);
  ...
  resizeListenters() {// Test size again
    // clearTimeout(this.expanding);
  ...
  display(request) {
    ...
    if (!this.displayed) {
      this.render(request).then(function () {
        this.emit(constants["c" /* EVENTS */].VIEWS.DISPLAYED, this);
  ...
  show() {
    ...
    this.emit(constants["c" /* EVENTS */].VIEWS.SHOWN, this);
  }
  ...
  hide() {
    ...
    this.emit(constants["c" /* EVENTS */].VIEWS.HIDDEN, this);
  }
...
event_emitter_default()(inline_InlineView.prototype);
/* harmony default export */ var inline = (inline_InlineView);
...
class rendition_Rendition {
  ...
  requireView(view) {
    ...
    } else if (typeof view == "string" && view === "inline") {
      View = inline;
    } else {
      // otherwise, assume we were passed a class function
      View = view;
...
/* harmony import */ var _utils_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(0);
/* harmony import */ var _epubcfi__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(2);
/* harmony import */ var _contents__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(9);
...
// EXTERNAL MODULE: ./src/rendition.js + 4 modules
